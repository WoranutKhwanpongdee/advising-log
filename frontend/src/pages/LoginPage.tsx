...
export default function LoginPage() {
  ...
  function handleSubmit(e: FormEvent) {
    ...
    if (!username.trim()) {
      setError(t('กรุณาระบุชื่อผู้ใช้งาน', 'Please enter your username'))
      return
    }
    if (!password) {
      setError(t('กรุณาระบุรหัสผ่าน', 'Please enter your password'))
      return
    }
    ...
    if (success) {
      ...
      setError(t('ไม่พบบัญชีผู้ใช้งาน กรุณาตรวจสอบชื่อผู้ใช้งานอีกครั้ง', 'User not found. Please check your username.'))
    }
  }
      ...
      <div className="w-full max-w-sm z-10">
        ...
        <div className="text-center mb-7">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-sky-50 dark:bg-sky-950/60 border border-sky-200/80 dark:border-sky-800 text-sky-700 dark:text-sky-300 text-xs font-semibold mb-4 shadow-2xs">
            ...
            {t('ระบบให้คำปรึกษาทางวิชาการและประกันคุณภาพการศึกษา', 'University Academic Advising System')}
          </div>
          ...
          <p className="text-sm sm:text-base text-slate-500 dark:text-slate-400 font-medium leading-relaxed">
            {t('ระบบบริหารจัดการการให้คำปรึกษาและติดตามคุณภาพการดูแลนักศึกษา', 'Institutional Student Success & Quality Assurance Platform')}
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/90 dark:border-slate-800 shadow-2xl dark:shadow-none overflow-hidden">
          <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-800 bg-slate-50/40 dark:bg-slate-800/50">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
              {t('กรุณาระบุชื่อผู้ใช้งานและรหัสผ่านเพื่อเข้าสู่ระบบ', 'Enter your username and password to continue')}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                placeholder={t('ตัวอย่างเช่น STU001 หรือ ADV001', 'e.g. STU001, ADV001')}
                value={username}
                onChange={e => { setUsername(e.target.value); setError('') }}
                className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-base text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-sky-500 transition-all"
              />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <input
                  ...
                  placeholder={t('รหัสผ่านสำหรับบัญชีทดสอบ', 'Any value accepted for now')}
                  value={password}
                  onChange={e => { setPassword(e.target.value); setError('') }}
                  className="w-full px-3.5 py-2.5 pr-10 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-base text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-sky-500 transition-all"
                />
                <button
                  type="button"
            ...
            {/* Error */}
            {error && (
              <p className="text-sm font-medium text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 rounded-xl px-3.5 py-2.5">
                {error}
              </p>
            )}
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-sky-600 hover:bg-sky-700 active:bg-sky-800 text-white text-base font-bold shadow-md shadow-sky-600/20 transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer mt-2"
            >
        ...
        {/* Hint */}
        <div className="mt-4 bg-sky-50/70 dark:bg-sky-950/40 border border-sky-100 dark:border-sky-900 rounded-2xl px-4 py-3 text-xs text-slate-500 dark:text-slate-400 space-y-0.5">
          ...
          <p className="mt-1 italic">{t('รหัสผ่านสำหรับบัญชีทดสอบ: สามารถระบุค่าใดก็ได้', 'Password: anything works')}</p>
        </div>

        <p className="text-center text-xs text-slate-400 mt-4 font-medium">
          {t('ระบบจำลองการยืนยันตัวตน · ข้อมูลทางการศึกษาที่ได้รับการคุ้มครอง', 'Protected Student Information · Local Authentication Simulation')}
        </p>
      </div>
    </div>
  )
