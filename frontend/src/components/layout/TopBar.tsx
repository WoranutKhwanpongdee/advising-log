...
const thaiNotificationTranslations: Record<string, { title: string; message: string }> = {
  NOT001: {
    title: 'กำหนดนัดหมายเรียบร้อยแล้ว',
    message: 'ระบบได้กำหนดนัดหมายเข้าพบอาจารย์ที่ปรึกษาของท่านในวันที่ 10 กันยายน 2569 เวลา 10:00 น. ณ ห้อง S2-301',
  },
  NOT002: {
    title: 'มีรายการที่ต้องดำเนินการติดตามผล',
    message: 'กรุณาจัดส่งแบบฟอร์มเพิ่มถอนรายวิชาภายในระยะเวลาที่กำหนด',
  },
  NOT003: {
    title: 'ดำเนินการติดตามผลเรียบร้อยแล้ว',
    message: 'แบบฟอร์มต่ออายุทุนการศึกษาของท่านได้รับการจัดส่งเรียบร้อยแล้ว',
  },
  NOT004: {
    title: 'มีคำร้องขอรับคำปรึกษาใหม่',
    message: 'Nattapong Wongchai (6631503003) ได้ยื่นคำร้องขอรับคำปรึกษาใหม่เกี่ยวกับประเด็นส่วนบุคคล',
  },
  NOT005: {
    title: 'มีคำร้องขอรับคำปรึกษาใหม่',
    message: 'Waraporn Chantara (6631503010) ได้ยื่นคำร้องขอรับคำปรึกษาใหม่เกี่ยวกับผลการเรียน',
  },
  NOT006: {
    title: 'กำหนดนัดหมายเรียบร้อยแล้ว',
    message: 'ระบบได้กำหนดนัดหมายเข้าพบอาจารย์ที่ปรึกษาของท่านในวันที่ 15 กันยายน 2569 เวลา 10:00 น. ณ ห้อง S2-205',
  },
  NOT007: {
    title: 'ใกล้ครบกำหนดส่งรายการติดตามผล',
    message: 'รายการติดตามผล “ส่งใบสมัครขอความช่วยเหลือทางการเงินฉุกเฉิน” ของท่านมีกำหนดส่งวันที่ 15 กันยายน',
  },
  NOT008: {
    title: 'มีคำร้องรอการดำเนินการ',
    message: 'Kannika Thongkam (6631503004) มีคำร้องขอรับคำปรึกษาเกี่ยวกับการฝึกงานหรืออาชีพที่รอการตรวจสอบ',
  },
}
    ...
    <header className="h-16 bg-white/90 dark:bg-[#0e1424]/90 backdrop-blur-md border-b border-slate-200/70 dark:border-slate-800/80 flex items-center justify-between px-3 sm:px-6 sticky top-0 z-30 shadow-xs text-slate-900 dark:text-slate-100">
      ...
      <div className="hidden lg:flex items-center gap-2 px-3.5 py-1.5 rounded-xl bg-slate-50/90 dark:bg-slate-800/60 border border-slate-200/70 dark:border-slate-700/60 shadow-2xs">
        ...
        <span className="text-xs font-bold text-slate-800 dark:text-slate-200">
          {t('ภาคการศึกษาที่ 1 ปีการศึกษา 2569', 'Semester 1 / 2026')}
        </span>
        <span className="text-slate-300 dark:text-slate-600">·</span>
        <span className="text-xs font-semibold text-sky-700 dark:text-sky-300 bg-sky-50 dark:bg-sky-500/12 px-2 py-0.5 rounded border border-sky-100 dark:border-sky-500/25">
          {t('ระบบบริการให้คำปรึกษาทางวิชาการ มฟล.', 'MFU SIS Advising')}
        </span>
      </div>
      ...
      <div className="flex items-center gap-1 sm:gap-2.5">
        ...
        <div className="flex items-center text-xs font-bold bg-slate-100 dark:bg-slate-800/80 p-0.5 rounded-xl border border-slate-200/70 dark:border-slate-700/60 shadow-2xs">
          <button
            type="button"
            ...
            <>
              ...
              <div className="absolute right-0 top-full mt-2.5 w-[calc(100vw-1.5rem)] max-w-88 bg-white dark:bg-[#0e1424] border border-slate-200/90 dark:border-slate-800 rounded-2xl shadow-2xl z-50 max-h-96 overflow-y-auto animate-[slideIn_0.15s_ease-out]">
                <div className="flex items-center justify-between px-5 py-3.5 border-b border-slate-100 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-900/60">
                  <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{t('การแจ้งเตือนจากระบบ', 'System Notifications')}</span>
                  {unreadCount > 0 && (
                    <button
                    ...
                    >
                      {t('ทำเครื่องหมายว่าอ่านทั้งหมดแล้ว', 'Mark all as read')}
                    </button>
                  )}
                </div>
                {myNotifs.length === 0 ? (
                  <p className="px-4 py-8 text-sm text-slate-400 dark:text-slate-500 text-center font-medium">{t('ไม่มีการแจ้งเตือนที่รอดำเนินการ', 'No pending notifications')}</p>
                ) : (
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between gap-2">
                          <p className={`text-sm ${n.isRead ? 'text-slate-600 dark:text-slate-400' : 'text-slate-900 dark:text-slate-100 font-bold'}`}>{translated?.title ?? n.title}</p>
                          {!n.isRead && <span className="h-2 w-2 rounded-full bg-sky-500 flex-shrink-0 mt-1 ring-2 ring-sky-100 dark:ring-sky-900" />}
                        </div>
                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 line-clamp-2 leading-relaxed">{translated?.message ?? n.message}</p>
                      </div>
                    )
                  })
                )}
